...
while true; do
  ...
  # Get metadata with yt-dlp when live stream is ready
  METADATA=$(yt-dlp --get-id --get-title --get-description \
    --no-playlist --playlist-items 1 "$LIVE_URL" --cookies www.youtube.com_cookies.txt 2>/dev/null)
